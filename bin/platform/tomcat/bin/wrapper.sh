...
# Optimized for use with version 3.5.29-pro of the Wrapper.
...
# daemon (without installing it) and when APP_NAME is longer than 14 chars. 
# In case you install the Wrapper as a service, make sure to set SKIP_SRC
# to null (empty value). 
SKIP_SRC=
...
launchinternal() {
    ...
    then 
        ...
        COMMAND_LINE="$CMDNICE \"$WRAPPER_CMD\" \"$WRAPPER_CONF\" wrapper.syslog.ident=\"$APP_NAME\" wrapper.pidfile=\"$PIDFILE\" wrapper.daemonize=TRUE $APPNAMEPROP $ANCHORPROP $IGNOREPROP $STATUSPROP $COMMANDPROP $LOCKPROP wrapper.script.version=3.5.29 $ADDITIONAL_PARA"
        eval $COMMAND_LINE
    else
...
console() {
    ...
    then
        ...
        # The string passed to eval must handles spaces in paths correctly.
        COMMAND_LINE="$CMDNICE \"$WRAPPER_CMD\" \"$WRAPPER_CONF\" wrapper.syslog.ident=\"$APP_NAME\" wrapper.pidfile=\"$PIDFILE\" $APPNAMEPROP $ANCHORPROP $STATUSPROP $COMMANDPROP $LOCKPROP wrapper.script.version=3.5.29 $ADDITIONAL_PARA"
        eval $COMMAND_LINE
    else
        eval echo `gettext '$APP_LONG_NAME is already running.'`
        exit 1
