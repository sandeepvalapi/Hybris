...
<body>
	...
	<xsl:for-each select="file">
	    <xsl:for-each select="violation">
			...
			<td style="padding: 3px" align="left" width="1%"><xsl:value-of disable-output-escaping="yes" select="@rule"/></td>
			<td style="padding: 3px" align="left" width="100%"><xsl:if test="@externalInfoUrl"><a><xsl:attribute name="href"><xsl:value-of select="@externalInfoUrl"/></xsl:attribute><xsl:call-template
			name="message"/></a></xsl:if><xsl:if test="not(@externalInfoUrl)"><xsl:call-template name="message"/></xsl:if></td>
		    </tr>
	    </xsl:for-each>
		    <tr><td>Next File</td></tr>	
    </xsl:for-each>
	</table>
	<br/>
